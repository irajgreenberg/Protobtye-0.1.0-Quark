	...
	*/
	...
	for (int i = 0; i < editableRingCount; ++i) { // rings
		...
		for (int j = 0; j < tubuleRingsIndices[i]; ++j) { // tubules in ring
			...
			tubules.push_back(Tube(Spline3(cps, tubuleInterpDetail, false, 1), random(.06, .22), tubuleDetail, ProtoTransformFunction(ProtoTransformFunction::LINEAR_INVERSE, Tup2f(.93, .12), 1),true));
	...
	theta = 0;
	float phi = 0;
	float radius = tubuleRadius*editableRingCount*1.04f;
	for(int i = 0; i < segs; ++i){
		radius = tubuleRadius*editableRingCount*random(1.09f, 1.11);
		...
		pts.push_back(Vec3f(sin(theta)*(radius + random(.02f, .04f)), -tubuleLen*.245 + step*.5375f*i + cos(phi)*random(.05f, .175f), cos(theta)*(radius + random(.02f, .04f))));
		theta += PI/segs*random(40, 60);
		phi += PI / 5;
	}
	...
	tubulesWrap = Tube(s, .7165, 18, ProtoTransformFunction(ProtoTransformFunction::SINUSOIDAL, Tup2f(.015, .079), 40), true);
	tubulesWrap.setColor(Col4f(.1f, .095f, .09f, 1));
}
